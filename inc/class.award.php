  ...
  public function getCandidate($Jahr, $Kategorie, $Gewinner = false) {
    ...
    foreach ($this->candIDs as $candID) {
      ...
      if( get_field('award_webseite', $candID )){
        $this->outputCandidate .= '<a href="' . get_field('award_webseite', $candID) . '" target="_blank" style="position:absolute; left:20px; bottom:17px;">';
      ...
      if( get_field('award_video', $candID )){
        $this->outputCandidate .= '<a href="' . get_field('award_video', $candID) . '" target="_blank" style="position:absolute; right:20px; bottom:17px;">';
        $this->outputCandidate .= '<div class="mc-button-neg se-mc-txt" style="margin:0px; border: solid 2px' . esc_attr( get_option( 'main_color_picker' ) ) . '; ">';
